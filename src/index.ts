...
import { showSettingsDialog, PluginSettings, DEFAULT_SETTINGS } from "./settings";
...
export default class TodoistSyncPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.boundBlockIconClick = this.onBlockIconClick.bind(this) as (event: CustomEvent) => void;
    ...
    uninstall() {
        this.removeData(STORAGE_KEY).catch((e: { msg?: string; message?: string }) => {
            showMessage(`Failed to remove settings: ${e.msg || e.message}`, -1, "error");
        });
    }
    ...
    openSetting() {
        showSettingsDialog(
            ...
            async (newSettings: PluginSettings) => {
                ...
                if (newSettings.syncInterval !== oldInterval || newSettings.apiToken !== oldToken) {
    ...
    private parseSlashKeywords(): string[] {
        const raw = this.settings.slashKeywords || "sendtotodoist, todoist, todo, task";
    ...
    private onBlockIconClick(event: CustomEvent<{
        ...
        detail.menu.addItem({
            iconHTML: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" style="margin-right: 4px;"><path fill="currentColor" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',
    ...
    private async sendBlockToTodoist(blockElements: HTMLElement[]) {
        if (!this.settings.apiToken) {
            showMessage(
                (this.i18n.apiKeyRequired as string) || "Please configure your Todoist API token in settings",
                -1,
                "error",
            );
            return;
    ...
    private async sendSingleBlockToTodoist(blockEl: HTMLElement, fromSlash = false) {
        if (!this.settings.apiToken) {
            showMessage(
                (this.i18n.apiKeyRequired as string) || "Please configure your Todoist API token in settings",
                -1,
        ...
        try {
            ...
            if (hasTodoistLink(kramdown)) {
                showMessage(
                    (this.i18n.taskAlreadySent as string) || "This block has already been sent to Todoist",
                    3000,
                    "info",
                );
                return;
            ...
            if (this.settings.projectName) {
                ...
                const match = projects.find(
                    (p) => p.name.toLowerCase() === this.settings.projectName.toLowerCase(),
                );
                if (match) {
                    projectId = match.id;
                } else {
                    showMessage(`Project "${this.settings.projectName}" not found in Todoist`, 3000, "error");
                }
            }
            ...
            if (this.settings.sectionName && projectId) {
                ...
                const match = sections.find(
                    (s) => s.name.toLowerCase() === this.settings.sectionName.toLowerCase(),
                );
                if (match) {
                    sectionId = match.id;
                } else {
                    showMessage(`Section "${this.settings.sectionName}" not found in Todoist`, 3000, "error");
                }
            }
            ...
            const todoistTask = await this.todoistClient.createTask({
                ...
                projectId,
                sectionId,
                labels: this.settings.defaultLabel ? [this.settings.defaultLabel] : undefined,
            });

            // Step 8: Add Todoist link back to block content
            const todoistUrl = todoistTask.url;
