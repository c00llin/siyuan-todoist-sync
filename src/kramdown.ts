...
export function isTaskBlock(kramdown: string): boolean {
    return /^[*-]\s+(\{:.*?\}\s*)?\[[ x]\]/.test(kramdown) || /^\[[ x]\]\s/.test(kramdown);
}
...
export function addTagToFirstLine(kramdown: string, tag: string): string {
    ...
    if (ialMatch) {
        const beforeIal = firstLine.substring(0, firstLine.length - ialMatch[0].length).trimEnd();
...
export function addTodoistLink(kramdown: string, todoistUrl: string): string {
    ...
    return kramdown.substring(0, insertPos) + " " + link + kramdown.substring(insertPos);
}
...
export function markTaskComplete(kramdown: string): string {
    ...
    if (linkMatch && linkMatch.index !== undefined) {
        ...
        if (!result.substring(afterLink).startsWith(" ✔︎")) {
            result = result.substring(0, afterLink) + " ✔︎" + result.substring(afterLink);
        }
    }
 ...
 */
export function removeSlashTrigger(kramdown: string, keywords: string[]): string {
    const { firstLine, rest } = splitFirstLine(kramdown);
    ...
    if (!slashMatch) {
        ...
        if (anySlash) {
            ...
            const isPrefix = keywords.some((kw) => kw.toLowerCase().startsWith(typed));
            if (isPrefix) {
                const newFirstLine = firstLine.replace(anySlash[0], "").replace(/\s{2,}/g, " ").trim();
                return joinLines(newFirstLine, rest);
            }
        }
    ...
    const isPrefix = typed.length === 0 || keywords.some((kw) => kw.toLowerCase().startsWith(typed));
    if (isPrefix) {
        const newFirstLine = firstLine.replace(slashMatch[0], "").replace(/\s{2,}/g, " ").trim();
        return joinLines(newFirstLine, rest);
    }
...
export function extractTaskContent(kramdown: string): string {
    ...
    const checkboxMatch = content.match(/^(?:[*-]\s+)?(?:\{:.*?\}\s*)?\[[ x]\]\s*/);
    if (checkboxMatch) {
